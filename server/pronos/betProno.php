<?php
  ...
  if ($_POST['userid'] == $_SESSION['userid']) {
    ...
    $queryprono = "SELECT COUNT(*) FROM pronos WHERE id_game=:id_game AND userid=:userid";
    $req = $bdd -> prepare($queryprono) or die(json_encode(array("status" => 500, "errorCode" => "BD", "message" => $bdd -> errorInfo())));
    $req -> execute(array('id_game' => $id_game, 'userid' => $userid));
    $result = $req -> fetch();
    ...
    if($result['COUNT(*)'] > 0){
      ...
      $query = "INSERT INTO pronos (userid,id_game,scoreA,scoreB,penalties,prono_date) VALUES (:userid,:id_game,:scoreA,:scoreB,:penalties,:prono_date)";
      $req = $bdd -> prepare($query) or die(json_encode(array("status" => 500, "errorCode" => "BD", "message" => $bdd -> errorInfo())));
      $req -> execute(array(
          'userid' => $userid,
          'id_game' => $id_game,
